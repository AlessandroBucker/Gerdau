...
export default function ParadasPage() {
  ...
  return <div>
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card">
      ...
      {loading ? <DataLoading label="Carregando paradas..." compact /> : <div className="overflow-x-auto"><table className="w-full min-w-[860px] text-left"><thead className="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500"><tr><th className="px-5 py-3">Área</th><th className="px-5 py-3">Tipo de parada</th><th className="px-5 py-3">Início</th><th className="px-5 py-3">Fim</th><th className="px-5 py-3 text-right">Editar</th></tr></thead><tbody className="divide-y divide-slate-200">{stops.map((item, index) => <tr key={`${item.area}-${index}`} onClick={() => item.id && router.push(`/ROTINA/paradas/${item.id}`)} className="cursor-pointer hover:bg-rose-50/50"><td className="px-5 py-4"><span className="rounded-lg bg-rose-50 px-2.5 py-1 text-sm font-bold text-rose-700">{item.area}</span></td><td className="px-5 py-4 font-semibold text-slate-800">{item.tipo}</td><td className="px-5 py-4"><DateTime date={item.inicio} time={item.horaInicio} /></td><td className="px-5 py-4"><DateTime date={item.fim} time={item.horaFim} /></td><td className="px-5 py-4 text-right"><button type="button" onClick={event => { event.stopPropagation(); setEditing(index); }} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700 hover:border-brand-200 hover:bg-brand-50 hover:text-brand-700" aria-label={`Editar ${item.tipo}`}><Pencil size={16} /> Editar</button></td></tr>)}</tbody></table></div>}
    </section>
