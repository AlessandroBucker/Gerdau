...
import { FileDown, ImagePlus, LoaderCircle, LockKeyhole, PackageCheck, Pencil, Plus, X } from "lucide-react";

type Conjunto = { ordem: string; equipamento: string; observacao: string; padrinho: string; recebimentoCentral: string; recebimentoArea: string; necessidade: string; status: string; prioridade?: string; comentarioCentral?: string };
...
export default function ConjuntosReservasPage() {
  ...
  function addImages(files: FileList | null) {
    ...
    const urls = Array.from(files).filter(file => file.type.startsWith("image/")).map(file => URL.createObjectURL(file));
    setImages(current => ({ ...current, [viewingIndex]: [...(current[viewingIndex] ?? []), ...urls] }));
  }
  ...
  function save(event: FormEvent<HTMLFormElement>) {
    ...
    if (editing === "new") setConjuntos(items => [...items, item]);
    else if (selectedIndex !== null) setConjuntos(items => items.map((current, index) => index === selectedIndex ? item : current));
    setEditing(null);
  }
  ...
  return (
    <div className="print-table-page">
        ...
        <div className="overflow-x-auto">
          <table className="w-full table-fixed text-left text-[10px] 2xl:text-[11px]">
            <thead className="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500">
              <tr>
                <th className="w-[8%] break-normal px-2 py-2">Prioridade do conjunto</th>
                <th className="w-[14%] break-normal px-1.5 py-2">Equipamentos e descrição breve</th>
                <th className="w-[19%] break-normal px-1.5 py-2">Observação da área para reforma</th>
                <th className="w-[8%] break-normal px-1.5 py-2">Padrinho da Laminação</th>
                <th className="w-[9%] break-normal px-1.5 py-2">Recebimento na Man. Central</th>
                <th className="w-[10%] break-normal px-1.5 py-2">Data do recebimento do serviço da área</th>
                <th className="w-[9%] break-normal px-1.5 py-2">Data de necessidade da área</th>
                <th className="w-[10%] break-normal px-1.5 py-2">Comentário da Manutenção Central</th>
                <th className="w-[6%] break-normal px-1.5 py-2">Status</th>
                <th className="w-[7%] whitespace-nowrap px-1.5 py-2">Ordem</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {conjuntos.map((item, index) => (
                <tr key={`${item.ordem}-${index}`} onClick={() => openDetails(index)} className={`cursor-pointer align-top transition ${selectedIndex === index ? "bg-brand-50 ring-1 ring-inset ring-brand-200" : "hover:bg-slate-50/80"}`}>
                  <td className="break-normal px-2 py-1.5 text-center text-slate-500">{valueOrDash(item.prioridade ?? "")}</td>
                  <td className="break-words px-1.5 py-1.5 font-semibold leading-4 text-slate-800">{item.equipamento}</td>
                  <td className="break-words px-1.5 py-1.5 leading-4 text-slate-600">{valueOrDash(item.observacao)}</td>
                  <td className="break-words px-1.5 py-1.5 font-semibold text-slate-700">{valueOrDash(item.padrinho)}</td>
                  <td className="break-words px-1.5 py-1.5 text-slate-600">{valueOrDash(item.recebimentoCentral)}</td>
                  <td className="break-words px-1.5 py-1.5 text-slate-600">{valueOrDash(item.recebimentoArea)}</td>
                  <td className="break-words px-1.5 py-1.5 font-semibold text-slate-700">{valueOrDash(item.necessidade)}</td>
                  <td className="break-words px-1.5 py-1.5 leading-4 text-slate-600">{valueOrDash(item.comentarioCentral ?? "")}</td>
                  <td className="break-words px-1.5 py-1.5">{item.status ? <span className={`inline-flex max-w-full break-words rounded px-1.5 py-0.5 text-[9px] font-bold uppercase leading-3 ${item.status === "Andamento" ? "bg-blue-50 text-blue-700" : "bg-amber-50 text-amber-700"}`}>{item.status}</span> : <span className="text-slate-300">—</span>}</td>
                  <td className="whitespace-nowrap px-1.5 py-1.5 font-mono font-bold text-brand-700">{item.ordem}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {viewingIndex !== null && conjuntos[viewingIndex] && <ConjuntoDetails item={conjuntos[viewingIndex]} images={images[viewingIndex] ?? []} onAddImages={addImages} onEdit={editFromDetails} onClose={() => setViewingIndex(null)} />}
...
function ConjuntoDetails({ item, images, onAddImages, onEdit, onClose }: { item: Conjunto; images: string[]; onAddImages: (files: FileList | null) => void; onEdit: () => void; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/55 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true"><div className="mx-auto min-h-full max-w-5xl rounded-2xl bg-white shadow-2xl"><header className="sticky top-0 z-10 flex items-center gap-3 rounded-t-2xl border-b border-slate-200 bg-white px-5 py-4 sm:px-7"><div className="min-w-0 flex-1"><p className="text-xs font-bold uppercase tracking-wider text-brand-600">Detalhes do conjunto</p><h2 className="truncate text-xl font-bold text-slate-900">{item.equipamento}</h2></div><button onClick={onEdit} className="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-brand-700"><Pencil size={16} /> Editar dados</button><button onClick={onClose} className="rounded-xl p-2.5 text-slate-400 hover:bg-slate-100" aria-label="Fechar"><X size={21} /></button></header><div className="p-5 sm:p-7"><section className="grid gap-px overflow-hidden rounded-xl border border-slate-200 bg-slate-200 sm:grid-cols-2">{details.map(([label, value]) => <div key={label} className="bg-white p-4"><p className="text-[11px] font-bold uppercase tracking-wide text-slate-400">{label}</p><p className="mt-1 whitespace-pre-wrap text-sm font-semibold leading-6 text-slate-700">{value || "—"}</p></div>)}</section><section className="mt-7"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-lg font-bold text-slate-900">Imagens do conjunto</h3><p className="text-sm text-slate-500">Registros visuais do equipamento e da reforma.</p></div><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-brand-200 bg-brand-50 px-4 py-2.5 text-sm font-bold text-brand-700 hover:bg-brand-100"><ImagePlus size={18} /> Adicionar imagem<input type="file" accept="image/*" multiple className="hidden" onChange={event => { onAddImages(event.target.files); event.target.value = ""; }} /></label></div>{images.length ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{images.map((source, index) => <div key={`${source}-${index}`} className="overflow-hidden rounded-xl border border-slate-200 bg-slate-50"><img src={source} alt={`Imagem ${index + 1} do conjunto`} className="h-52 w-full object-cover" /></div>)}</div> : <div className="grid min-h-44 place-items-center rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 text-center"><div><ImagePlus className="mx-auto text-slate-300" size={30} /><p className="mt-2 text-sm font-semibold text-slate-500">Nenhuma imagem adicionada</p></div></div>}</section></div></div></div>;
}
...
function ConjuntoForm({ item, onClose, onSubmit }: { item: Conjunto | null; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <Modal title={item ? "Editar conjunto" : "Incluir conjunto"} onClose={onClose} icon={item ? <Pencil size={19} /> : <Plus size={20} />}><form onSubmit={onSubmit} className="grid max-h-[70vh] gap-3 overflow-y-auto pr-1 sm:grid-cols-2"><Field label="Prioridade" name="prioridade" defaultValue={item?.prioridade} required={false} /><Field label="Ordem" name="ordem" defaultValue={item?.ordem} /><div className="sm:col-span-2"><Field label="Equipamento e descrição" name="equipamento" defaultValue={item?.equipamento} /></div><div className="sm:col-span-2"><Field label="Observação da área" name="observacao" defaultValue={item?.observacao} required={false} /></div><Field label="Padrinho da Laminação" name="padrinho" defaultValue={item?.padrinho} required={false} /><Field label="Status" name="status" defaultValue={item?.status} required={false} /><Field label="Recebimento na Man. Central" name="recebimentoCentral" defaultValue={item?.recebimentoCentral} required={false} /><Field label="Recebimento do serviço da área" name="recebimentoArea" defaultValue={item?.recebimentoArea} required={false} /><Field label="Necessidade da área" name="necessidade" defaultValue={item?.necessidade} required={false} /><div className="sm:col-span-2"><Field label="Comentário da Manutenção Central" name="comentarioCentral" defaultValue={item?.comentarioCentral} required={false} /></div><div className="flex gap-2 sm:col-span-2"><button type="button" onClick={onClose} className="flex-1 rounded-xl border border-slate-200 px-4 py-2.5 font-bold text-slate-600">Cancelar</button><button className="flex-1 rounded-xl bg-brand-600 px-4 py-2.5 font-bold text-white">Salvar</button></div></form></Modal>;
}
