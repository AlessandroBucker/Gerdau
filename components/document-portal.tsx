      ...
      <section className="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-card sm:rounded-3xl">
        <div className="p-6 sm:p-10">
          ...
          <form className="mt-8" onSubmit={submit} noValidate>
            ...
            <input ref={inputRef} id="access-code" name="access-code" value={code} onChange={(event) => updateCode(event.target.value)} inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} placeholder="000000" aria-invalid={Boolean(error)} aria-describedby={error ? "access-error" : "access-help"} className={`w-full rounded-xl border bg-slate-50 px-3 py-4 text-center text-xl font-semibold tracking-[0.3em] text-slate-900 transition placeholder:text-slate-300 sm:px-4 sm:text-2xl sm:tracking-[0.35em] ${error ? "border-red-400 ring-4 ring-red-50" : "border-slate-200 focus:border-brand-500 focus:ring-4 focus:ring-brand-50"}`} />
