...
export function EventsDashboard() {
  ...
  const visibleEvents = useMemo(() => events.filter(event => filters[event.type] && event.end >= isoDate(days[0]) && event.start <= isoDate(days[days.length - 1])), [days, events, filters]);
  ...
  return (
    <div>
      ...
      {loading ? <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card"><DataLoading label="Carregando eventos do calendário..." /></section> : view === "calendar" ? <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card">
