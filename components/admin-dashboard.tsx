...
export function AdminDashboard() {
  ...
  async function createUser(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await api("/api/admin/codes", {
        ...
        body: JSON.stringify({ personalNumber: form.get("personalNumber"), description: form.get("description") }),
      });
      formElement.reset();
  ...
  async function remove(code: AccessCode) {
    if (!confirm(`Excluir o Número Pessoal ${code.numero_pessoal} e todos os PDFs vinculados? Esta ação não pode ser desfeita.`)) return;
    setBusy(code.id);
    setMessage(null);
    try {
      ...
      <div className="grid gap-7 xl:grid-cols-[1.55fr_0.8fr]">
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card">
          <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
            <div><h2 className="flex items-center gap-2 text-lg font-bold text-slate-900"><KeyRound className="text-brand-600" size={20} /> Números Pessoais</h2><p className="mt-1 text-sm text-slate-500">Cadastre e controle quem pode acessar documentos.</p></div>
          ...
          <form onSubmit={createUser} className="grid gap-3 border-b border-slate-100 bg-slate-50/60 p-5 sm:grid-cols-[150px_1fr_auto]">
            <input required name="personalNumber" inputMode="numeric" pattern="[0-9]{8}" maxLength={8} placeholder="Número Pessoal" aria-label="Número Pessoal com 8 dígitos" className="rounded-xl border border-slate-200 bg-white px-4 py-3" />
            <input required name="description" maxLength={200} placeholder="Nome Completo SAP" aria-label="Nome Completo SAP" className="rounded-xl border border-slate-200 bg-white px-4 py-3" />
            <button disabled={busy === "create"} className="rounded-xl bg-brand-600 px-5 py-3 font-semibold text-white hover:bg-brand-700 disabled:opacity-60">Cadastrar</button>
          </form>
              ...
              <article key={code.id} className="p-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="font-mono text-xl font-bold tracking-wider text-slate-900">{code.numero_pessoal}</p>
          ...
          <div className="hidden overflow-x-auto md:block">
            <table className="w-full min-w-[650px] text-left">
              <thead className="bg-slate-50 text-xs uppercase tracking-wider text-slate-500"><tr><th className="px-5 py-3">Número Pessoal</th><th className="px-5 py-3">Nome Completo SAP</th><th className="px-5 py-3">Criação</th><th className="px-5 py-3">Status</th><th className="px-5 py-3 text-right">Ações</th></tr></thead>
              <tbody className="divide-y divide-slate-100">
                {loading ? <tr><td colSpan={5} className="px-5 py-12 text-center text-slate-400"><LoaderCircle className="mx-auto animate-spin" /></td></tr> : codes.length === 0 ? <tr><td colSpan={5} className="px-5 py-12 text-center text-slate-400">Nenhum usuário cadastrado.</td></tr> : codes.map(code =>
                  <tr key={code.id} className="hover:bg-slate-50/70">
                    <td className="px-5 py-4 font-mono text-lg font-bold tracking-wider text-slate-800">{code.numero_pessoal}</td>
        ...
        <section className="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-card">
          ...
          <p className="mt-1 text-sm leading-6 text-slate-500">Envie até 20 PDFs por vez, com até 50 MB cada, para o Número Pessoal selecionado.</p>
          <form onSubmit={upload} className="mt-6 space-y-5">
            <label className="block text-sm font-semibold text-slate-700">Número Pessoal<select required name="codeId" defaultValue="" className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3"><option value="" disabled>Selecione um Número Pessoal</option>{codes.filter(code => code.ativo).map(code => <option key={code.id} value={code.id}>{code.numero_pessoal} — {code.descricao}</option>)}</select></label>
